...
# def get_adj_AQI_b():
#     df = pd.read_csv("./data/air_quality/beijing.csv")
...
# def get_adj_AQI_t():
#     df = pd.read_csv("./data/air_quality/tianjin.csv")
#     df = df[['latitude', 'longitude']]
#     res = geographical_distance(df, to_rad=False).values
#     adj = get_similarity_AQI(res)
#     print(adj)
...
# def get_similarity_discharge(df, thr=0.1, include_self=False, force_symmetric=False, sparse=False):
#     data = df.drop(columns=["date"]).values.T  # (num_stations, num_timesteps)
    
#     corr_matrix = np.corrcoef(data)    
#     dist_matrix = 1 - corr_matrix

#     theta = np.std(dist_matrix)
#     adj = thresholded_gaussian_kernel(dist_matrix, theta=theta, threshold=thr)
...
#     return adj
def get_similarity_ssc():
    df = pd.read_csv("./data/hydrology/SSC_sites_flow_direction.csv", index_col=0)
    node_ids = df.columns.tolist()  # Ensure consistent node ID order
    adj = df.to_numpy().astype(float)  # shape [N, N]
    return adj
