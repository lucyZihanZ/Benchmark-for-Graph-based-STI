...
# data shifting: add noises
# def seasonal_drift_AQI(data, drift_strength=0.1):
#     T = data.shape[0]
#     drift = np.sin(np.linspace(0, 2*np.pi, T))[:, None, None] * drift_strength
#     return data + drift
...
def train_da(
    ...
    for epoch_no in range(config["epochs"]):
        ...
        with tqdm(src_train_loader, mininterval=5.0, maxinterval=50.0) as src_it, \
            ...
            for src_batch, tgt_batch in zip(src_it, tgt_it):
                if waveblend_interpolation==1:
                    ...
                    # assert_no_nan(torch.tensor(local_in_trg), "valid wavelet_interpolation output")
                ...
                # assert_no_nan(src_batch["coeff_frequency"], "src_batch['coeff_frequency']")
                # assert_no_nan(tgt_batch["coeff_frequency"], "tgt_batch['coeff_frequency']")
                ...
                if miu_ntc != 0: 
                    with torch.no_grad():
                        augmented_tgt_mse_loss, augmented_tgt_pred, augmented_tgt_feat, augmented_tgt_masks = src_model(tgt_batch)                   
                        consistency_loss = (augmented_tgt_pred - tgt_pred).abs().mean()
                        print(f"[DEBUG] consistency_loss: {consistency_loss.item()}, miu_ntc: {miu_ntc}")
                        total_loss += miu_ntc * consistency_loss
                ...
                for p in src_model.parameters():
                    if p.grad is not None:
                        ...
                        total_norm_src += param_norm.item() ** 2
                total_norm_src = total_norm_src ** 0.5
                # print(f"[DEBUG] Src grad norm: {total_norm_src}")
                # Compute tgt_model grad norm
                total_norm_tgt = 0
                for p in tgt_model.parameters():
                    if p.grad is not None:
                        param_norm = p.grad.data.norm(2)
                        total_norm_tgt += param_norm.item() ** 2
                total_norm_tgt = total_norm_tgt ** 0.5
                # print(f"[DEBUG] Tgt grad norm: {total_norm_tgt}")
        ...
        if tgt_valid_loader is not None and (epoch_no + 1) % valid_epoch_interval == 0:
            ...
            with torch.no_grad():   
                with tqdm(src_valid_loader, desc=f"Validation Epoch {epoch_no}", mininterval=5.0, maxinterval=50.0) as valid_src_it, \
                    tqdm(tgt_valid_loader, desc="Test Phase", mininterval=5.0, maxinterval=50.0) as valid_tgt_it:
                    for valid_src_batch, valid_tgt_batch in zip(valid_src_it, valid_tgt_it):
                        if waveblend_interpolation==1:
                            ...
                            assert_no_nan(valid_tgt_batch["coeff_frequency"], "valid_tgt_batch['coeff_frequency']")
                        ...
                        assert_no_nan(loss, "tgt_mse_loss")
    ...
    with torch.no_grad():
        ...
        with tqdm(src_test_loader, mininterval=5.0, maxinterval=50.0) as src_test_it, \
            ...
            for src_test_batch, tgt_test_batch in zip(src_test_it, tgt_test_it):
                ...
                # assert_no_nan(tgt_test_batch["coeff_frequency"], "tgt_batch['coeff_frequency']")
                # assert_no_nan(tgt_test_batch["observed_data"], "tgt_batch['observed_data']")
