...
from DGDI_model import graph_diff
...
# unconditional/conditional diffusion process:
    ...
    def set_input_to_diffmodel(self, noisy_data, observed_data, observed_mask, cond_mask, itp_info):
        ...
        elif self.config['model']['waveblend_interpolation']==True:
            cond_obs = (cond_mask * observed_data).unsqueeze(1)
            noisy_target = ((observed_mask - cond_mask) * noisy_data  + (1-observed_mask)*itp_info).unsqueeze(1) # (B, 1, K, L)
    ...
    def impute(self, observed_data, observed_mask, cond_mask, side_info, n_samples, itp_info):
        ...
        #     imputed_mean = imputed_samples.mean(dim=1)  # (B, K, L)

        #     # reshape for spatial layer
        #     B, K, L = imputed_mean.shape
        #     T = self.num_steps  # for temporal information  
        #     N = K // T
        #     D = L
        #     assert K % T == 0
        #     imputed_samples = imputed_mean.view(B, T, N, D)

        # # # feed into GraphTransformer / temporal attention
        # # imputed_samples = self.spatial_layer(imputed_mean_reshaped)
    ...
    def forward(self, batch, is_train=1):
        ...
        itp_info = None
        if self.waveblend_interpolation:
            itp_info = coeffs
