...
def get_block_mask(observed_mask, target_strategy='block'):
    ...
    print("cond_mask ratio (==1):", cond_mask.sum() / cond_mask.numel())
    return cond_mask
...
def sample_mask(shape, p=0.0015, p_noise=0.05, max_seq=1, min_seq=1, rng=None):
    ...
    print("eval_mask True number:", np.sum(mask))
    print("eval_mask:", mask) # test whether mask all equal to 0.
    return mask.astype('uint8')


class Pems04_Dataset(Dataset):
    def __init__(self, seed = 123, eval_length=24, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
...
def get_dataloader_pems04(batch_size, device, val_len=0.1, test_len=0.2, missing_pattern='block',num_workers=1, target_strategy='hybrid'):
    dataset = Pems04_Dataset(seed = 123, mode="train", val_len=val_len, test_len=test_len, missing_pattern=missing_pattern,
