...
class Pems08_Dataset(Dataset):
    def __init__(self, eval_length=24, mode="train", val_len=0.1, test_len=0.2, missing_pattern='block',
        ...
        df = np.load("./data/pems/pems08_new.npz")
        data = df['data']
        df = pd.DataFrame(data)
        date_range = pd.date_range(start='2016-07-01', end ='2016-08-30 23:59:59', freq='h')
        ...
        path = "./data/pems/pems07_meanstd.pk"
...
def get_dataloader_pems08(batch_size, device, val_len=0.1, test_len=0.2, missing_pattern='block', num_workers=1, target_strategy='hybrid'):
    dataset = Pems08_Dataset(mode="train", val_len=val_len, test_len=test_len, missing_pattern=missing_pattern,
                            target_strategy=target_strategy)
    train_loader = DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers, shuffle=True
    )
    dataset_test = Pems08_Dataset(mode="test", val_len=val_len, test_len=test_len, missing_pattern=missing_pattern,
                                target_strategy=target_strategy)
    test_loader = DataLoader(
        dataset_test, batch_size=batch_size, num_workers=num_workers, shuffle=False
    )
    dataset_valid = Pems08_Dataset(mode="valid", val_len=val_len, test_len=test_len, missing_pattern=missing_pattern,
                                target_strategy=target_strategy)
