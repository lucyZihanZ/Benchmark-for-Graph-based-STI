...
class AirQuality_Dataset(Dataset):
    def __init__(self, eval_length=36, target_dim=27, mode="train", target_strategy="random", validindex=0, missing_ratio=0.1, seed=1):
        ...
        data = pd.read_csv("data/air_quality/beijing.csv",index_col="time",parse_dates=True) #8760*27
        data.index = pd.to_datetime(data.index)
        selected_data = data[Beijing_attributes]
        ...
        path = "data/air_quality/beijing_air_quality_meanstd.pk"
        with open(path, "wb") as f:
            pickle.dump([self.mean, self.std], f)

        df = pd.read_csv("data/air_quality/beijing.csv",index_col="time",parse_dates=True)         
...
def get_dataloader_Beijing(device, seed=1, target_strategy="random", batch_size=16, missing_ratio=0.1, validindex=0, missing_pattern=None):
...
# Get dataloaders
train_loader, valid_loader, test_loader, scaler, mean_scaler = get_dataloader_Beijing(
    device=device,
    seed=42,
...
# Print some batches from train_loader
print("Testing train_loader...")
for batch in train_loader:
    print("Batch keys:", batch.keys())
    print("observed_data shape:", batch["observed_data"].shape)
    print("observed_mask shape:", batch["observed_mask"].shape)
    print("gt_mask shape:", batch["gt_mask"].shape)
    print("hist_mask shape:", batch["hist_mask"].shape)
    print("cond_mask shape:", batch["cond_mask"].shape)
    print("cut_length:", batch["cut_length"])
    print("timepoints:", batch["timepoints"])
    break  # just test 1 batch

# Print some batches from valid_loader
print("\nTesting valid_loader...")
for batch in valid_loader:
    print("observed_data shape:", batch["observed_data"].shape)
    break  # test 1 batch

# Print some batches from test_loader
print("\nTesting test_loader...")
...
print("Mean scaler shape:", mean_scaler.shape)