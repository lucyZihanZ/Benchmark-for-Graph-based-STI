...
parser.add_argument(
    "--dataset", type=str, default="pems", choices=["air_quality",  "pems", "ssc"]
)
parser.add_argument(
...
parser.add_argument("--miu_using_mse", type=int, default=1)
parser.add_argument("--miu_using_multi_domain", type=int, default=0)
...
########## src and tgt dataloader
    src_train_loader, src_valid_loader, src_test_loader, src_scaler, src_mean_scaler = get_dataloader_pems08(
    device=args.device,
...
# train_loader, valid_loader, test_loader, scaler, mean_scaler
    tgt_train_loader, tgt_valid_loader, tgt_test_loader, tgt_scaler, tgt_mean_scaler = get_dataloader_pems04(
    device=args.device,
    target_strategy=args.targetstrategy,
    missing_pattern=args.missing_pattern,
    batch_size=config["train"]["batch_size"],
...
# if args.dataset == "hydrology":
#     src_train_loader, src_valid_loader, src_test_loader, src_scaler, src_mean_scaler = get_dataloader_pooled(
#     device=args.device,
#     target_strategy=args.targetstrategy,
#     batch_size=config["train"]["batch_size"],
#     missing_pattern=args.missing_pattern)
#     adj_src = get_similarity_pems08()
# # train_loader, valid_loader, test_loader, scaler, mean_scaler
#     tgt_train_loader, tgt_valid_loader, tgt_test_loader, tgt_scaler, tgt_mean_scaler = get_dataloader_discharge(
#     device=args.device,
#     target_strategy=args.targetstrategy,
#     missing_pattern=args.missing_pattern,
#     batch_size=config["train"]["batch_size"],
#     )
if args.dataset == "ssc":
    src_train_loader, src_valid_loader, src_test_loader, src_scaler, src_mean_scaler = get_dataloader_discharge(
    device=args.device,
    target_strategy=args.targetstrategy,
    batch_size=config["train"]["batch_size"],
    missing_pattern=args.missing_pattern)
    adj_src = get_similarity_ssc()
# train_loader, valid_loader, test_loader, scaler, mean_scaler
    tgt_train_loader, tgt_valid_loader, tgt_test_loader, tgt_scaler, tgt_mean_scaler = get_dataloader_pooled(
    device=args.device,
    target_strategy=args.targetstrategy,
    missing_pattern=args.missing_pattern,
    batch_size=config["train"]["batch_size"],
    )
    adj_tgt = get_similarity_ssc()
