...
class PriSTI_WaterQuality(PriSTI):
    def __init__(self, config, device, target_dim=20, seq_len=None): # Default target_dim to 20
        # If seq_len is dynamic from data, it should be passed from outside.
        # If it's fixed (e.g., window size), set it here or via config.
        # Assuming seq_len will be determined by your dataloader's batch.
        super(PriSTI_WaterQuality, self).__init__(target_dim, seq_len, config, device)
        self.config = config # Storing config in self.config is redundant if super() does it, but harmless.
    ...
    def process_data(self, batch):
        ...
        # Permute from [B, L, K] to [B, K, L] (Batch, Nodes/Features, Length/TimeSteps)
        # where K is 20 for your water quality data
