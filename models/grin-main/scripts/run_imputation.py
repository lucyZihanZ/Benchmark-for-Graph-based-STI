...
# from fillers import britsfiller, filler, graphfiller, multi_imputation_filler, rgainfiller
# from datasets import water_quality
# # import config  # 如果你把 config.py 从 lib 拿出来放到顶层
# import config as global_config_module

# from data_process.spatiotemporal import SpatioTemporalDataModule
# from data_process.imputation_dataset import ImputationDataset, GraphImputationDataset

# from nn import models
# from nn.utils.metric_base import MaskedMetric
# from nn.utils.metrics import MaskedMAE, MaskedMAPE, MaskedMSE, MaskedMRE

# from utils_files import parser_utils, numpy_metrics, utils
# from utils_files.parser_utils import str_to_bool
...
#     if model_str == 'grin':
#         model, filler = models.GRINet, graphfiller.GraphFiller
#     if model_str == 'brits':
#         model, filler = models.BRITSNet, britsfiller.BRITSFiller
#     elif model_str == 'grin':
#         model, filler = models.GRINet, graphfiller.GraphFiller
#     elif model_str == 'mpgru':
#         model, filler = models.MPGRUNet, graphfiller.GraphFiller
#     elif model_str == 'bimpgru':
#         model, filler = models.BiMPGRUNet, graphfiller.GraphFiller
#     elif model_str == 'var':
#         model, filler = models.VARImputer, filler.Filler
#     elif model_str == 'gain':
#         model, filler = models.RGAINNet, rgainfiller.RGAINFiller
#     elif model_str == 'birnn':
#         model, filler = models.BiRNNImputer, multi_imputation_filler.MultiImputationFiller
#     elif model_str == 'rnn':
#         model, filler = models.RNNImputer, filler.Filler
#     else:
...
#     if dataset_name == 'ssc_block':
#         dataset = water_quality.MissingValuesWaterQuality()
#     elif dataset_name == 'ssc_point':
#         dataset = water_quality.MissingValuesWaterQuality(p_fault=0., p_noise=0.25)
...
#     aggr_methods = utils.ensure_list(args.aggregate_by)
#     df_hats = utils.prediction_dataframe(y_hat, index, dataset.df.columns, aggregate_by=aggr_methods)
...
import torch
import torch.nn.functional as F
import yaml # This is for loading YAML files
from pytorch_lightning.callbacks import EarlyStopping, ModelCheckpoint
from pytorch_lightning.loggers import TensorBoardLogger
from torch.optim.lr_scheduler import CosineAnnealingLR
import sys
...
from fillers import britsfiller, filler, graphfiller, multi_imputation_filler, rgainfiller
from datasets import water_quality
# import config  # <-- REMOVE THIS LINE, or be very careful about naming conflicts
...
# IMPORTANT: To use your config.py, you should import it with an alias
# to avoid naming conflicts with the 'config' argument or variable
import config as global_config_module # <-- Import your config.py with an alias
...
def get_model_classes(model_str):
    ...
    if model_str == 'brits':
        model, filler = models.BRITSNet, britsfiller.BRITSFiller
    elif model_str == 'grin': # Duplicate 'grin' entry, consider removing
        model, filler = models.GRINet, graphfiller.GraphFiller
    elif model_str == 'mpgru':
        model, filler = models.MPGRUNet, graphfiller.GraphFiller
    elif model_str == 'bimpgru':
...
def parse_args():
    ...
    parser.add_argument("--dataset-name", type=str, default='ssc_point')
    parser.add_argument("--config", type=str, default=None) # Path to YAML config
    # Splitting/aggregation params
    parser.add_argument('--in-sample', type=str_to_bool, nargs='?', const=True, default=False)
    parser.add_argument('--val-len', type=float, default=0.1)
    parser.add_argument('--test-len', type=float, default=0.2)
    ...
    args = parser.parse_args()
    # This block loads the YAML config and merges it into args
    if args.config is not None:
        with open(args.config, 'r') as fp:
            config_args = yaml.load(fp, Loader=yaml.FullLoader)
        for arg in config_args:
...
def run_experiment(args):
    ...
    # Access the 'logs' path from your imported global_config_module
    # global_config_module.config is the dictionary defined in config.py
    # Then access the 'logs' key from that dictionary.
    logdir = os.path.join(global_config_module.config['logs'], args.dataset_name, args.model_name, exp_name)

    # save config for logging
    pathlib.Path(logdir).mkdir(parents=True, exist_ok=True)
    with open(os.path.join(logdir, 'config.yaml'), 'w') as fp:
        # Note: This saves the ARGS into config.yaml, not the content of global_config_module.
        yaml.dump(parser_utils.config_dict_from_args(args), fp, indent=4, sort_keys=True)
    ...
    #                             window=args.window,
    #                             stride=args.stride)
    torch_dataset = dataset_cls(dataset.dataframe.values, # Pass the NumPy values from the dataframe
                                mask=dataset.training_mask,
                                eval_mask=dataset.eval_mask,
                                window=args.window,
                                stride=args.stride)
    ...
    filler.load_state_dict(torch.load(checkpoint_callback.best_model_path,
                                      map_location=lambda storage, loc: storage)['state_dict']) # Use map_location for loading
    filler.freeze()
    trainer.test()
    filler.eval()
    ...
    index = dm.torch_dataset.data_timestamps(dm.testset.indices, flatten=False)['horizon']
    aggr_methods = utils.ensure_list(args.aggregate_by)
    df_hats = utils.prediction_dataframe(y_hat, index, dataset.df.columns, aggregate_by=aggr_methods) # Ensure dataset.df.columns is accessible
    df_hats = dict(zip(aggr_methods, df_hats))
    for aggr_by, df_hat in df_hats.items():
        # Compute error
        print(f'- AGGREGATE BY {aggr_by.upper()}')
