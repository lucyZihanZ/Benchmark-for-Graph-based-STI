...
from utils_files.utils import ensure_list
...
class Filler(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.scheduler_class is not None:
            ...
            scheduler_dict = {
                'scheduler': scheduler,
                'interval': 'epoch',     # 通常是 epoch，也可以改为 'step'
                'frequency': 1           # 每多少个interval调用一次
            }

            if metric is not None:
                scheduler_dict['monitor'] = metric

            cfg['lr_scheduler'] = scheduler_dict
