...
from imputeformer.datasets.ssc import WaterQuality
...
def get_dataset(dataset_name: str):
    print(f"[Debug] Received dataset_name: {dataset_name}")
    ...
    elif dataset_name.endswith('ssc'):
        p_fault, p_noise = 0., 0.25
    else:
    ...
    if dataset_name == 'cer':
        return add_missing_values(CEREn(), p_fault=p_fault, p_noise=p_noise,
                                  min_seq=12, max_seq=12 * 4, seed=56789)
    if dataset_name == 'ssc':
        return add_missing_values(WaterQuality(), p_fault=p_fault, p_noise = p_noise, min_seq = 12, max_seq=12 * 4, seed = 45678)
...
def parse_args():
    ...
    parser.add_argument("--dataset-name", type=str, default='ssc')
