...
#         return torch.sqrt(F.mse_loss(y_hat, y, reduction='none'))
import torch
import torch.nn.functional as F
from tsl.nn.metrics.metric_base import MaskedMetric  # 你自己的基类路径
...
class MaskedRMSE(MaskedMetric):
    def __init__(self,
                 ...
                 dist_sync_fn=None,
                 at=None):
        # 正确传入 metric_fn：它必须是 element-wise 的
        def rmse_fn(y_hat, y):
            mse = F.mse_loss(y_hat, y, reduction='none')
            return torch.sqrt(mse + 1e-8)  # 避免 sqrt(0)
        ...
        super().__init__(
            ...
            process_group=process_group,
            dist_sync_fn=dist_sync_fn,
            metric_kwargs={},  # 不传 reduction，否则你自定义函数会报错
            at=at
        )
