...
class CSDI_base(nn.Module):
    ...
    def calc_loss_valid(
        self, observed_data, observed_data_interpolation,cond_mask, observed_mask, side_info, is_train
    ):
        loss_sum = 0
        for t in range(self.num_steps):  # calculate loss for all t
            loss = self.calc_loss(
                observed_data, observed_data_interpolation, cond_mask, observed_mask, side_info, is_train, set_t=t
            )
            loss_sum += loss.detach()
        return loss_sum / self.num_steps

    def calc_loss(
        self, observed_data, observed_data_interpolation, cond_mask, observed_mask, side_info, is_train, set_t=-1
    ):
        ...
        noise = torch.randn_like(observed_data_interpolation)
        noisy_data = (current_alpha ** 0.5) * observed_data_interpolation + (1.0 - current_alpha) ** 0.5 * noise

        total_input = self.set_input_to_diffmodel(noisy_data, observed_data_interpolation, observed_data, cond_mask)
    ...
    def set_input_to_diffmodel(self, noisy_data, observed_data_interpolation, observed_data, cond_mask):
        if self.is_unconditional == True:
            total_input = noisy_data.unsqueeze(1)  # (B,1,K,L)
        else:
            cond_obs = (cond_mask * observed_data_interpolation).unsqueeze(1)
            noisy_target = ((1 - cond_mask) * noisy_data).unsqueeze(1)
            total_input = torch.cat([cond_obs, noisy_target], dim=1)
        return total_input

    def set_input_to_diffmodel_z(self, noisy_z, observed_data, cond_mask):
            cond_obs_z = (cond_mask * observed_data).unsqueeze(1)
            noisy_target = (noisy_z).unsqueeze(1)
            # total_input = torch.cat([cond_obs, noisy_target], dim=1)  # (B,2,K,L)
            return cond_obs_z, noisy_target




    def impute(self, observed_data, observed_data_interpolation, cond_mask, side_info, n_samples):
        B, K, L = observed_data.shape
    ...
    def forward(self, batch, is_train=1):
        ...
        # observed_tp1 = observed_tp.cpu().detach().numpy()

        side_info = self.get_side_info(observed_tp, cond_mask)

        # side_info11 = side_info.cpu().detach().numpy()

        loss_func = self.calc_loss if is_train == 1 else self.calc_loss_valid

        return loss_func(observed_data, observed_data_interpolation, cond_mask, observed_mask, side_info, is_train)
    ...
    def evaluate(self, batch, n_samples):
        ...
        with torch.no_grad():
            ...
            # fsdafsdfs = gt_mask.cpu().numpy()

            target_mask = observed_mask - cond_mask

            side_info = self.get_side_info(observed_tp, cond_mask)

            samples = self.impute(observed_data, observed_data_interpolation, cond_mask, side_info, n_samples)
...
class CSDI_pems(CSDI_base):
    def __init__(self, config, device, target_dim=325):
        super(CSDI_pems, self).__init__(target_dim, config, device)
...
class CSDI_ptbxl(CSDI_base):
    def __init__(self, config, device, target_dim=1):
        super(CSDI_ptbxl, self).__init__(target_dim, config, device)
