...
class VAE_base(nn.Module):
    ...
    def set_input_to_vae(self, observed_data, observed_data_interpolation, cond_mask):
        total_input = observed_data_interpolation.unsqueeze(1)
        return total_input


    def reconstruct(self, observed_data, observed_data_interpolation, cond_mask, side_info):
        VAE_input = (observed_data_interpolation).unsqueeze(1)
...
class VAE_pems(VAE_base):
    def __init__(self, config, device, target_dim=325):
        super(VAE_pems, self).__init__(target_dim, config, device)
