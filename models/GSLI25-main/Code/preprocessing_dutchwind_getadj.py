...
def get_similarity(dist, thr=0.05, include_self=False, force_symmetric=False, sparse=False):
...
def get_adj_dutchwind():
    df = pd.read_csv("Data/dutchwind_stations.csv", header=None)
    df = df[[1,2]]
...
adj = get_adj_dutchwind()
np.savetxt("Data/dutchwind/dutchwind_adj.csv", adj, delimiter=",", fmt="%.6f")
